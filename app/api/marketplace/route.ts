...
export async function GET(request: Request) {
  ...
  const products = await db.execute(sql`SELECT p.id, p.name, p.description, p.category, p.price, CASE WHEN p."imageUrl" LIKE 'blob:%' OR p."imageUrl" = '' THEN '/placeholder.jpg' ELSE p."imageUrl" END AS "imageUrl", p.state, p.availability, sp."shopName", sp."avatarUrl", sp.bio, u.name AS "sellerName" FROM products p LEFT JOIN seller_profiles sp ON sp."userId" = p."sellerId" LEFT JOIN "user" u ON u.id = p."sellerId" WHERE ${sql.join(productFilters, sql` AND `)} ORDER BY p."createdAt" DESC`)
