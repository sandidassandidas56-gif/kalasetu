...
export default function AuthPage() {
  ...
  async function signInWithSocial(provider: SocialProvider) {
    ...
    if (result.error) {
      setPending(false)
      setMessage(`${provider === 'github' ? 'GitHub' : 'Google'} sign-in is not configured yet.`)
    }
  }
    ...
    <main className="min-h-screen bg-[#f7f2e9] px-6 py-12 text-[#20352f]">
      <div className="mx-auto grid max-w-5xl overflow-hidden rounded-[2rem] bg-white shadow-xl lg:grid-cols-2">
        ...
        <section className="p-8 sm:p-12 lg:p-16">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2"><button type="button" disabled={pending || !databaseReady} onClick={() => void signInWithSocial('google')} className="rounded-full border border-[#d8d1c4] px-6 py-3 font-bold disabled:opacity-60">Continue with Google</button><button type="button" disabled={pending || !databaseReady} onClick={() => void signInWithSocial('github')} className="rounded-full border border-[#d8d1c4] px-6 py-3 font-bold disabled:opacity-60">Continue with GitHub</button></div>
