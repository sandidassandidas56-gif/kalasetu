...
export default async function SellerProductsPage() {
  ...
  const rows = !hasDatabaseConnection() ? [] : ((await db.execute(sql`SELECT p.id, p.name, p.category, p.price, p.availability, p.published FROM products p WHERE p."sellerId" = ${session.user.id} ORDER BY p."createdAt" DESC`)) as unknown as { rows?: Record<string, unknown>[] }).rows ?? []
