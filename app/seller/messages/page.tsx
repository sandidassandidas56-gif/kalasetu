...
export default async function SellerMessagesPage() {
  ...
  let result: { rows?: { body: string; createdAt: string; sender_name: string }[] } = { rows: [] }
  try { result = !hasDatabaseConnection() ? { rows: [] } : await db.execute(sql`SELECT m.body, m."createdAt", COALESCE(u.name, 'Buyer') AS sender_name FROM messages m LEFT JOIN "user" u ON u.id = m."senderId" WHERE m."recipientId" = ${session.user.id} ORDER BY m."createdAt" DESC LIMIT 50`) as typeof result } catch { result = { rows: [] } }
