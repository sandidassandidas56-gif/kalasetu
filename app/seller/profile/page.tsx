...
export default async function SellerProfilePage() {
  ...
  if (hasDatabaseConnection()) {
    ...
    const initial = { shopName: String(profile.shopName ?? ''), state: String(profile.state ?? ''), bio: String(profile.bio ?? ''), address: String(profile.address ?? ''), phone: String(profile.phone ?? '') }
