...
export function ensureMarketplaceSchema() {
  ...
  marketplaceSchemaReady ??= db.execute(sql`CREATE TABLE IF NOT EXISTS seller_profiles (
      ...
      "createdAt" timestamptz NOT NULL DEFAULT now(), "updatedAt" timestamptz NOT NULL DEFAULT now(),
      UNIQUE ("buyerId", "productId")
    )`)).then(() => db.execute(sql`CREATE UNIQUE INDEX IF NOT EXISTS user_email_lower_unique_idx ON "user" (lower(email))`)).then(() => db.execute(sql`CREATE INDEX IF NOT EXISTS products_seller_id_idx ON products ("sellerId")`)).then(() => db.execute(sql`CREATE INDEX IF NOT EXISTS products_published_idx ON products (published)`)).then(() => db.execute(sql`CREATE INDEX IF NOT EXISTS marketplace_orders_buyer_idx ON marketplace_orders ("buyerId")`)).then(() => db.execute(sql`CREATE INDEX IF NOT EXISTS marketplace_order_items_seller_idx ON marketplace_order_items ("sellerId")`)).then(() => db.execute(sql`CREATE INDEX IF NOT EXISTS inquiries_seller_idx ON inquiries ("sellerId")`)).then(() => db.execute(sql`CREATE INDEX IF NOT EXISTS notifications_user_idx ON notifications ("userId")`)).then(() => db.execute(sql`CREATE INDEX IF NOT EXISTS reviews_product_idx ON reviews ("productId")`)).then(() => undefined)
